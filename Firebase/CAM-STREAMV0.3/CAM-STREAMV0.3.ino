        ...
        if (!fb) {
            ...
            if(!detection_enabled || fb->width > 400){
                ...
                /**********************************************/
                  ...
                  fb_data_t fb1;
                  fb1.width = image_matrix->w;
                  fb1.height = image_matrix->h;
                  fb1.data = image_matrix->item;
                  fb1.bytes_per_pixel = 3;
                  fb1.format = FB_BGR888;
                  fb_gfx_print(&fb1, 1, 1, FACE_COLOR_YELLOW, "Antonio");
                /**********************************************/
                if (!image_matrix) {
                    Serial.println("dl_matrix3du_alloc failed");
                    res = ESP_FAIL;
                    ...
                    if(!fmt2rgb888(fb->buf, fb->len, fb->format, image_matrix->item)){
                        ...
                        if (net_boxes || fb->format != PIXFORMAT_JPEG){
                            if(net_boxes){
                                ...
                                draw_face_boxes(image_matrix, net_boxes);  //繪製人臉方框
                                dl_lib_free(net_boxes->score);
                                dl_lib_free(net_boxes->box);
                                dl_lib_free(net_boxes->landmark);
                                dl_lib_free(net_boxes);                                
